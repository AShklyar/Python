...
users, hobby, base = sys.argv[1:]
print(users, hobby, base)
with open(base, 'w', encoding='utf-8') as f_base:  # открываем файл через менеджер контекста на запись и
    # работаем в области его видимости
    with open(users, encoding='utf-8') as f_users:  # открыли на чтение файл пользователей
        with open(hobby, encoding='utf-8') as f_hobby:  # открываем на чтение файл с хобби
