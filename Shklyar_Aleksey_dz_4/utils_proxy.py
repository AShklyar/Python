...
def currency_rates(some_cash):
    """ Function takes some currency and returns current values from site cbr.ru
     some_cash str type
     """
    proxies = {
        'http': 'http://10.2.0.40:8090'
    }
    session = requests.session()
    session.proxies.update(proxies)

    get_val =  session.get('http://www.cbr.ru/scripts/XML_daily.asp')
